...
func TestUserModelGet(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Initialize a connection pool to our test database, add defer and
			// call to the teardown function, so it's always run immediately
			// Before this sub test returns.
			db, teardown := newTestDB(t)
			defer teardown()

			// Create a new instance of te UserModel.
			m := UserModel{db}

			// Call the UserModel.Get() method and check that te return value
